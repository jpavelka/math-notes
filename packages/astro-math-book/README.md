...
All result types — environments, notation symbols, and full-text prose matches — appear in a single ranked list. The most relevant result surfaces first regardless of its source.

### Registry search (always available)

The modal instantly searches all numbered environments (theorems, definitions, lemmas, equations, figures, etc.) and notation symbols directly from the registry built at compile time. Results appear as you type, with an expandable content preview below each environment and notation entry.

Results are scored by how closely the query matches the entry's label, title, type+number, aliases, and body text.

### Full-text prose search (production only)

When a production build is served, the modal also queries the full page text via the [Pagefind](https://pagefind.app) JavaScript API after a short debounce. Prose matches are interleaved into the ranked list and tagged **In text**.

> Full-text search requires the Pagefind index (`/pagefind/`) generated by `npm run build`. It is unavailable in `npm run dev` — only environment and notation search run in development.
...
**Definitions default to their title.** A `<Ref>` to a `Definition` entry renders its title (the defined term) as the link text by default, since definitions are usually cited by the term they introduce rather than by number. It falls back to `"Definition N.M"` when the definition has no title.

```mdx
{/* def:combOpt has title "Combinatorial Optimization" */}
See <Ref id="def:combOpt" /> for the formal definition.
{/* renders: "See Combinatorial Optimization for the formal definition." */}
```

Use `useEnvNum` to force the `"Type N.M"` form as the link text, overriding the title default (and any custom `label`). This is the mirror of `useTitle`:

```mdx
See <Ref id="def:combOpt" useEnvNum />.
{/* renders: "See Definition 2.1." */}
```

`useEnvNum` works for any referenceable environment, but is most useful for definitions (whose default is the title). The hover tooltip is unaffected.
...
| `annotation` | `string \| ReactNode` | Optional annotation; supports `$...$` inline math and `[ref:id]` / `[ref:id\|alt label]` cross-references. Pass `ReactNode` only from `.tsx` — MDX files must use strings (Astro's JSX runtime is incompatible with React children). |
